...
async function getSalesData(startDate, endDate) {
  const salesData = await Order.aggregate([ // perform analysis on all kind of data //
    {
      // Stage 1 - Filter results
      $match: {
        createdAt: {
          $gte: new Date(startDate), /// greater then and less then // 
...
function getDatesBetween(startDate, endDate) {
  ...
  while (currentDate <= new Date(endDate)) {
    ...
    currentDate.setDate(currentDate.getDate() + 1); // incrementing date //
  }
...
export const getSales = catchAsyncErrors(async (req, res, next) => {
  ...
  endDate.setUTCHours(23, 59, 59, 999);  //both for one day //
